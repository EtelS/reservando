...
Listado.prototype.buscarRestaurante = function(id) {
    if ( listadoDeRestaurantes.length > id && id > 0){ 
    let resultado= this.restaurantes.find(restaurant=>restaurant.id===id);
    return resultado;
  } else{
...
function quitarRepetidos(arreglo){
    var noRepetidos=arreglo.filter(function(elem,index,self){
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    var ciudad = this.restaurantes.map(function(restaurantes){
        return restaurantes.ubicacion;
    })
    return quitarRepetidos(ciudad).sort();
}
...
Listado.prototype.obtenerRubros = function() {
    var rubros= this.restaurantes.map(function(restaurantes){
        return restaurantes.rubro;
    })
    return quitarRepetidos(rubros).sort();
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    return quitarRepetidos(horario).sort();
}
